...
import { IOSConfig, WarningAggregator, withAppDelegate, withInfoPlist } from '@expo/config-plugins';
import eol from 'eol';
/*
 * Mods for Info.plist
 */
...
export const modifyAppDelegateSwift = config => {
  ...
  // Step 3 If openUrlIdentifierElementIndex do not exist in AppDelegate.swift add a warning that something went wrong
    ...
    WarningAggregator.addWarningIOS('withExpoBraintree', `Unable to find  "options: [UIApplication.OpenURLOptionsKey: Any] = [:]" in AppDelegate.swift, automatic changes not applied expo-braintree might not working as expected`);
  }
  ...
  if (!~expoBraintreeConfigHandleUrlElementIndex) {
    WarningAggregator.addWarningIOS('withExpoBraintree', `Unable to find  "return ExpoBraintreeConfig.handleUrl(url: url)" in AppDelegate.swift, automatic changes not applied expo-braintree might not working as expected`);
  }
  return contents;
};
...
export const modifyAppDelegateObjectiveC = (config, xCodeProjectAppName) => {
  if (!xCodeProjectAppName) {
    WarningAggregator.addWarningIOS('withExpoBraintree', `xCodeProjectAppName props not provided but in case of using plugin in objective c world it is required`);
  }
  const appDelegate = config.modResults;
  let contents = eol.split(appDelegate.contents);
  ...
  return withAppDelegate(expoConfig, config => {
    ...
    switch (appDelegateLanguage) {
      ...
      case 'swift':
        ...
        return config;
      default:
        WarningAggregator.addWarningIOS('withExpoBraintree', `${appDelegateLanguage} AppDelegate file is not supported yet`);
        return config;
    }
  });
};
...
export const withBraintreeWrapperFile = (config, {
  appDelegateLanguage
}) => {
