...
exports.withExpoBraintreeAndroidGradle = exports.withExpoBraintreeAndroid = exports.appendContents = exports.addPaypalAppLinks = void 0;
...
const withExpoBraintreeAndroid = (expoConfig, {
  ...
  newConfig = (0, _configPlugins.withMainActivity)(expoConfig, config => {
    ...
    if (initializePayPal === 'true') {
      newSrc.push(`   ExpoBraintreeModule.init()${language === 'java' ? ';' : ''}`);
    }
    if (initializeVenmo === 'true') {
      newSrc.push(`   ExpoBraintreeModule.initVenmo()${language === 'java' ? ';' : ''}`);
    }
...
const addPaypalAppLinks = (modResults, host, pathPrefix) => {
  ...
  if (!host) {
    throw Error('No Host provided for withExpoBraintree.android addPaypalAppLinks');
  }
