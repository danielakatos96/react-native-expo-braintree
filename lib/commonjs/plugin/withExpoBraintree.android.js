...
exports.withExpoBraintreeAndroidGradle = exports.withExpoBraintreeAndroid = exports.appendContents = exports.addBraintreeLinks = void 0;
...
const addBraintreeLinks = (modResults, host, pathPrefix, addFallbackUrlScheme, initialize3DSecure) => {
  const mainActivity = getMainActivityOrThrow(modResults);
  const intentFilters = mainActivity['intent-filter'];

  // Host is required props for a plugin
  if (!host) {
    _configPlugins.WarningAggregator.addWarningAndroid('withExpoBraintree addBraintreeLinks', `No Host provided for withExpoBraintree.android addBraintreeLinks`);
  }

  // Check if the intent filter already exists
  if (hasIntentFilter(intentFilters, host, pathPrefix)) {
    _configPlugins.WarningAggregator.addWarningAndroid('withExpoBraintree addBraintreeLinks', `withExpoBraintreeAndroid: AndroidManifest not require any changes`);
    return modResults;
  }
 ...
 */
exports.addBraintreeLinks = addBraintreeLinks;
...
const withExpoBraintreeAndroidGradle = expoConfig => {
  return (0, _configPlugins.withProjectBuildGradle)(expoConfig, config => {
...
exports.withExpoBraintreeAndroidGradle = withExpoBraintreeAndroidGradle;
